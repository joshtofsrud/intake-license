#!/bin/bash
# transfer-tenant-scope-fix — inventory audit finding #1 (cross-tenant write).
#   SERVICE (TransferRequestService):
#     · create() asserts the inventory item, destination location, explicit
#       source location, and sale all belong to the requesting tenant before
#       anything is written — existence is not authorization
#     · the from-location auto-suggest is tenant-scoped (it previously
#       searched EVERY tenant's stock rows and could pick a foreign location)
#     · markSent / markReceived take the tenant from the transfer row itself
#       instead of inferring it from a related record, and re-assert that the
#       item and location belong to it — so even a row poisoned before this
#       fix can no longer move another tenant's stock
#   CONTROLLER (RegisterController), both oversell endpoints:
#     · inventory_item_id exists-rules are tenant-scoped
#     · sale_id (transfer) and customer_id (special order) are tenant-scoped —
#       customer_id was previously passed straight through to
#       SpecialOrderService, allowing a foreign customer attachment
#   No routes, no migrations, no schema change. Legitimate transfers behave
#   identically; invalid ones now fail validation or throw.
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-TRANSFER-SCOPE" app/Services/Tenant/TransferRequestService.php; then
  echo "transfer-tenant-scope-fix already applied — aborting."; exit 1
fi
if ! grep -q "MARKER-SPLIT-TENDER" app/Http/Controllers/Tenant/RegisterController.php; then
  echo "wrong base (split-tender missing from RegisterController) — aborting."; exit 1
fi

cat > 'app/Services/Tenant/TransferRequestService.php' <<'TSCOPE_0_EOF'
<?php
...
cat > 'app/Http/Controllers/Tenant/RegisterController.php' <<'TSCOPE_1_EOF'
<?php
...
echo "transfer-tenant-scope-fix applied — server: git pull (no migrate, no view:clear needed)"
