#!/bin/bash
# booking-reset-placement — two fixes to the start-over control:
#   · placement: it docks right-aligned on the ACTIVE step's title row
#     (next to "Pick a drop-off date" etc.) with a subtle border, instead
#     of floating detached in the far corner of the progress bar
#   · visibility: appears on every screen past the first — the Items
#     pre-step and all main steps — not just from Schedule on (the old
#     check used the internal step counter, which starts at Services)
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-RESET-PLACEMENT" public/js/booking.js; then
  echo "already applied — aborting."; exit 1
fi
if ! grep -q "MARKER-FUNNEL-SESSION-FIX" public/js/booking.js; then
  echo "wrong base — aborting."; exit 1
fi

cat > 'resources/views/public/booking.blade.php' <<'RESETPL_0_EOF'
...
cat > 'public/js/booking.js' <<'RESETPL_1_EOF'
/**
...
echo "reset placement applied — view:clear on server"
