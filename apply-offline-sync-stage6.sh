#!/bin/bash
# offline-sync-stage-6 — status row moves to just below the user name in the
# sidebar; mobile dot + gear re-aligned to the header bell (proper SVG gear,
# matched height, real spacing). Cache-buster → stage6.
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "v=stage6" resources/views/layouts/tenant/app.blade.php; then
  echo "offline-sync-stage-6 already applied — aborting."; exit 1
fi
if ! grep -q "v=stage5" resources/views/layouts/tenant/app.blade.php; then
  echo "stage 5 not applied — aborting."; exit 1
fi

cat > 'public/js/offline-sync.js' <<'OFS6_0_EOF'
...
cat > 'resources/views/layouts/tenant/app.blade.php' <<'OFS6_1_EOF'
<!DOCTYPE html>
...
<script src="{{ asset('js/offline-sync.js') }}?v=stage6"></script>
...
cat > 'resources/views/layouts/tenant/_sidebar.blade.php' <<'OFS6_2_EOF'
@php
...
echo "offline-sync-stage-6 applied — view:clear on server, hard-refresh browsers"
