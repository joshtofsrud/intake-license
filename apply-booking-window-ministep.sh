#!/bin/bash
# booking-window-ministep — B2 from the approved mockup. The pickup-window
# choice becomes a focused mini-step after the date: stepper (date done ->
# window pending), radio cards with hover/selected states and spots-left,
# plus a dashed "None of these work — reach out to me" skip that satisfies
# the required choice, clears any window, and sends pickup_outreach in the
# booking payload (server side lands in pickup-outreach-alerts). Refresh
# restore re-applies a saved window or skip choice.
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-WINDOW-MINISTEP" public/js/booking.js; then
  echo "booking-window-ministep already applied — aborting."; exit 1
fi
if ! grep -q "MARKER-ITEMS-PICK" public/js/booking.js; then
  echo "booking-items-pick not applied — wrong base, aborting."; exit 1
fi

cat > 'public/js/booking.js' <<'WINMINI_0_EOF'
/**
...
(function () {
  ...
  function renderPdWindows(dateStr) {
    ...
    sub.textContent = 'Pick one window — this is how your ' + (window.BkAssetSingular || 'item') + ' reaches us.';
...
cat > 'resources/views/public/booking.blade.php' <<'WINMINI_1_EOF'
...
echo "booking-window-ministep applied — view:clear on server"
