...
function renderSplit() {
  ...
  cart.payments.forEach((p, i) => {
    ...
    row.innerHTML = '<span>' + (p.label || p.method) + '</span>'
      + '<span class="amt">' + fmt(p.amount_cents) + '</span>'
      + '<span class="x">✕</span>'
      + (p.change_cents ? '<span class="chg">Change due ' + fmt(p.change_cents) + '</span>' : '');
    row.querySelector('.x').addEventListener('click', () => { cart.payments.splice(i, 1); renderSplit(); });
...
document.querySelectorAll('#tenderModal .reg-tender-btn').forEach(btn => {
  btn.addEventListener('click', () => {
    ...
    // MARKER-SPLIT-STRIPE — mid-split Stripe card is the closing leg: no
    // amount entry (it takes the remainder), no confirm step.
    if (btn.dataset.tender === 'card' && ROUTES.directPaymentsEnabled && ROUTES.directPaymentsPk && cart.payments.length > 0) {
    ...
    (function () {
      ...
      const splittable = !((t === 'card' && ROUTES.directPaymentsEnabled && ROUTES.directPaymentsPk) || t === 'payment_link' || t === 'mark_paid');
...
async function openCardPaymentModal() {
  ...
  // MARKER-SPLIT-STRIPE — in a split, the card leg charges exactly what's left.
  const amountCents = cart.payments.length > 0
    ? splitRemaining()
...
async function confirmCardPayment() {
  ...
  // MARKER-SPLIT-STRIPE — split closing leg: the charged card becomes the
  // final payments[] row and the commit goes out as a split.
  if (cart.payments.length > 0) {
    cart.payments.push({
      method: 'card',
      amount_cents: DirectPay.chargeAmountCents || splitRemaining(),
      change_cents: 0,
  ...
  try {
    ...
    if (hasRefund) {
      ...
      payload = {
        ...
        payment_method: cart.payment_method,
        payment_reference: cart.payment_reference,
        payments: cart.payments.length ? cart.payments.map(p => ({ method: p.method, amount_cents: p.amount_cents, reference: p.reference })) : null, // MARKER-SPLIT-TENDER
