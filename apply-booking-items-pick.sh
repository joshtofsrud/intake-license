#!/bin/bash
# booking-items-pick — A1 from the approved mockup. Returning customers'
# account items render as explicit pick-cards (check circle, tap to toggle);
# nothing is auto-added, deselect replaces remove so items are always
# recoverable, Continue is gated with a hint until at least one item is
# selected. Manually added items keep their editable input + Remove.
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-ITEMS-PICK" public/js/booking.js; then
  echo "booking-items-pick already applied — aborting."; exit 1
fi
if ! grep -q "MARKER-BOOKING-RESET" public/js/booking.js; then
  echo "booking-reset not applied — wrong base, aborting."; exit 1
fi

cat > 'public/js/booking.js' <<'ITEMSPICK_0_EOF'
/**
...
cat > 'resources/views/public/booking.blade.php' <<'ITEMSPICK_1_EOF'
...
echo "booking-items-pick applied — view:clear on server (booking.js is filemtime-busted)"
