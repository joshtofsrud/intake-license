...
# apply-invest-site.sh — MARKER-INVEST-SITE
# Ports the investment-site mockup into the app as a token-gated page at /invest/{token}.
# New files only, plus one append to routes/web.php. No existing code is rewritten.

echo "==> checking repo root"
test -f artisan || { echo "run this from the intake-license repo root"; exit 1; }

if grep -q "MARKER-INVEST-SITE" routes/web.php 2>/dev/null; then
  echo "MARKER-INVEST-SITE already present in routes/web.php — nothing to do."
  exit 0
fi

echo "==> creating directories"
mkdir -p resources/views/invest storage/app/invest database/migrations app/Models app/Http/Controllers app/Console/Commands

echo "==> migration"
cat > database/migrations/2026_08_12_190000_create_invest_tables.php <<'MIGEOF'
<?php
...
echo "==> models"
cat > app/Models/InvestToken.php <<'TOKEOF'
<?php
...
cat > app/Models/InvestLead.php <<'LEADEOF'
<?php
...
echo "==> controller"
cat > app/Http/Controllers/InvestController.php <<'CTLEOF'
<?php
...
cat > app/Console/Commands/InvestTokenCommand.php <<'CMDEOF'
<?php
...
echo "==> blade view"
cat > resources/views/invest/page.blade.php <<'VIEWEOF'
@verbatim
<!DOCTYPE html>
<html lang="en"><head>
<meta charset="utf-8">
...
echo ""
echo "MARKER-INVEST-SITE applied."
echo "  files: migration, InvestToken, InvestLead, InvestController, InvestTokenCommand, invest/page.blade.php"
echo "  routes/web.php appended"
echo ""
echo "After deploy, on the server:"
echo "  php artisan intake:invest-token            # prints the live link"
echo "  php artisan intake:invest-token --rotate   # kills old links, issues a new one"
echo ""
echo "Drop the four PDFs into storage/app/invest/ on the server to enable /doc links."
