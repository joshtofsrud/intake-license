#!/usr/bin/env bash
# apply-436-campaigns.sh — Sales campaigns (channels) + pipeline quoting.
# Patches 436 (channels), 437 (prospect channel/quote fields + pricing config),
# 438 (panel registration), 439 (channel seeder).
#
# PREREQUISITE: the sales-channel package (patches 430–435) must already be
# applied — this script edits files that package created and anchors on lines
# patch-432 added to AdminPanelProvider.
#
# Run from the repo root:  bash apply-436-campaigns.sh
# Idempotent: guarded on MARKER-PATCH-437 in config/sales_quoting.php.
set -euo pipefail

[ -f artisan ] || { echo "ERROR: run from the Laravel repo root (artisan not found)."; exit 1; }
[ -f app/Models/SalesProspect.php ] || { echo "ERROR: sales-channel package (430–435) not applied yet."; exit 1; }
if [ -f config/sales_quoting.php ] && grep -q MARKER-PATCH-437 config/sales_quoting.php; then
  echo "apply-436-campaigns.sh: already applied — skipping."; exit 0
fi

echo "Applying patches 436–439 …"

# ─────────────────────────────────────────────────────────────────────────────
# config/sales_quoting.php  (MARKER-PATCH-437)
# ─────────────────────────────────────────────────────────────────────────────
mkdir -p config
cat > config/sales_quoting.php <<'EOF'
<?php
// MARKER-PATCH-437 — Quote pricing for the sales pipeline.
...
# ─────────────────────────────────────────────────────────────────────────────
# migration: sales_channels  (MARKER-PATCH-436)
# ─────────────────────────────────────────────────────────────────────────────
cat > database/migrations/2026_07_07_000001_create_sales_channels_table.php <<'EOF'
<?php
// MARKER-PATCH-436 — Campaign/channel definitions per vertical.
...
# ─────────────────────────────────────────────────────────────────────────────
# migration: prospect channel + quote fields  (MARKER-PATCH-437)
# ─────────────────────────────────────────────────────────────────────────────
cat > database/migrations/2026_07_07_000002_add_channel_and_quote_to_sales_prospects.php <<'EOF'
<?php
// MARKER-PATCH-437 — Prospects join a channel and carry a built quote.
...
# ─────────────────────────────────────────────────────────────────────────────
# app/Models/SalesChannel.php  (MARKER-PATCH-436)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# Filament: SalesChannelResource + pages  (MARKER-PATCH-436)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# seeder  (MARKER-PATCH-439)
# ─────────────────────────────────────────────────────────────────────────────
cat > database/seeders/SalesChannelSeeder.php <<'EOF'
<?php
// MARKER-PATCH-439 — Bike shops channel (active) + Salons (draft).
...
"""    public function tenant(): BelongsTo
    ...
    // MARKER-PATCH-437 — channel + quote
...
# panel registration (MARKER-PATCH-438) — anchors on lines patch-432 added
P = "app/Providers/Filament/AdminPanelProvider.php"
s = rd(P)
if "MARKER-PATCH-438" in s:
    print("  panel registration already applied — skipping.")
else:
    edit(P,
    "use App\\Filament\\Resources\\SalesProspectResource;",
    "use App\\Filament\\Resources\\SalesChannelResource; // MARKER-PATCH-438\nuse App\\Filament\\Resources\\SalesProspectResource;")
    edit(P,
    "                SalesProspectResource::class,",
    "                SalesChannelResource::class, // MARKER-PATCH-438\n                SalesProspectResource::class,")
