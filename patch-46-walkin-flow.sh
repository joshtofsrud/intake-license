#!/bin/bash
# ============================================================================
# patch-46-walkin-flow.sh
# ----------------------------------------------------------------------------
# Mobile v1.1 — Walk-in flow MVP.
#
# Implements the walk-in counter flow per the 51026-intake-mobile-mvp mockup
# (screen 9). When a customer walks up:
#
#   FAB tap → "Who's at the counter?" search screen
#            → Existing customer (search/recent)        → choice screen
#            → New customer (quick capture: name+phone) → choice screen
#            → Quick retail sale (no customer)          → /admin/register
#   Choice → Book appointment → service+time picker → submits to existing
#                                                     quick-book.store endpoint
#            → Quick sale → /admin/register?customer_id=X (pre-attached)
#
# Architecture:
#   - New WalkInController with index() returning a stateful Blade view
#   - SPA-style page with vanilla JS state machine
#   - Reuses existing endpoints: /customers/search, /customers POST,
#     /calendar/quick-book GET + POST, /admin/register
#   - Modifies RegisterController::index() to accept ?customer_id= and
#     pre-attach the customer to the cart (one-line addition)
#   - Replaces FAB onclick toast with navigation to /admin/register/walk-in
#
# Files created:
...
#
# Files modified:
#   resources/views/layouts/tenant/_mobile-fab.blade.php (route the FAB)
#   app/Http/Controllers/Tenant/RegisterController.php (accept ?customer_id=)
#   resources/views/tenant/register/index.blade.php (pre-attach passed customer)
#   routes/web.php (add walk-in routes)
#
# Deploy:
...
if [ ! -f "artisan" ] || [ ! -f "app/Http/Controllers/Tenant/RegisterController.php" ]; then
  echo "ERROR: not in project root" >&2
  exit 1
fi

echo "==> Patch 46: Walk-in flow MVP"
echo ""

# ----------------------------------------------------------------------------
# 1. WalkInController
# ----------------------------------------------------------------------------
cat > app/Http/Controllers/Tenant/WalkInController.php <<'CTRL'
<?php
...
CTRL
echo "    CREATED app/Http/Controllers/Tenant/WalkInController.php"

# ----------------------------------------------------------------------------
# 2. Walk-in view (the SPA-ish flow)
# ----------------------------------------------------------------------------
mkdir -p resources/views/tenant/walkin
cat > resources/views/tenant/walkin/index.blade.php <<'VIEW'
...
@endsection
VIEW
echo "    CREATED resources/views/tenant/walkin/index.blade.php"

# ----------------------------------------------------------------------------
# 3. Routes — add walk-in routes
# ----------------------------------------------------------------------------
python3 <<'PYEOF'
...
if "WalkInController" in s:
    print("    SKIP routes/web.php — walk-in already added")
else:
    # Anchor: the register routes block.
    anchor = "Route::get('/register',                  [TenantControllers\\RegisterController::class, 'index'])->name('register.index');"
    if s.count(anchor) != 1:
        raise SystemExit(f"ABORT: register route anchor count = {s.count(anchor)}")

    new = anchor + "\n            Route::get('/register/walk-in',          [TenantControllers\\WalkInController::class, 'index'])->name('register.walk-in');"
    s = s.replace(anchor, new, 1)

    p.write_text(s)
    print("    UPDATED routes/web.php — added /register/walk-in route")
PYEOF

# ----------------------------------------------------------------------------
# 4. FAB — replace toast with navigation to walk-in route
# ----------------------------------------------------------------------------
python3 <<'PYEOF'
from pathlib import Path
p = Path("resources/views/layouts/tenant/_mobile-fab.blade.php")
s = p.read_text()

if "register.walk-in" in s:
    print("    SKIP _mobile-fab.blade.php — already routed")
else:
    old_onclick = 'onclick="window.IntakeToast && window.IntakeToast.info(\'Walk-in flow ships in v1.1. For now, use New appointment.\')"'
    # Note: the Blade {{ route() }} call needs UNESCAPED single quotes inside.
    # Python single-quoted string lets us write them literally.
    new_onclick = "onclick=\"window.location.href='{{ route('tenant.register.walk-in', ['subdomain' => $currentTenant->subdomain]) }}'\""

    if s.count(old_onclick) != 1:
        raise SystemExit(f"ABORT: FAB onclick anchor count = {s.count(old_onclick)}")

    s = s.replace(old_onclick, new_onclick, 1)
    p.write_text(s)
    print("    UPDATED _mobile-fab.blade.php — FAB now navigates to walk-in flow")
PYEOF

# ----------------------------------------------------------------------------
# 5. RegisterController + register view — accept ?customer_id= and pre-attach
# ----------------------------------------------------------------------------
python3 <<'PYEOF'
from pathlib import Path

p = Path("app/Http/Controllers/Tenant/RegisterController.php")
s = p.read_text()

marker = "Patch 46: pre-attach customer from query"
if marker in s:
    print("    SKIP RegisterController — already accepts customer_id query")
else:
    # Anchor: the return view('tenant.register.index'... line in index().
    old = "        return view('tenant.register.index', ["
    new = '''        // Patch 46: pre-attach customer from query param (walk-in flow).
    ...
    if s.count(old) != 1:
        raise SystemExit(f"ABORT: RegisterController index anchor count = {s.count(old)}")
    s = s.replace(old, new, 1)

    # Also pass $preAttachCustomer into the view payload — find the closing
    # of the view-data array and inject before the closing.
    # The simplest: append a new key after 'tenant' => $tenant,
    old2 = "'tenant'     => $tenant,"
    ...
    p.write_text(s)
    print("    UPDATED RegisterController — accepts ?customer_id=, passes preAttachCustomer")
PYEOF

# 5b. Register view: pre-attach the customer to cart on load if passed
python3 <<'PYEOF'
from pathlib import Path
p = Path("resources/views/tenant/register/index.blade.php")
s = p.read_text()

marker = "// Patch 46: pre-attach"
if marker in s:
    print("    SKIP register/index.blade.php — already has pre-attach hook")
else:
    # Find an anchor near the cart init — look for the script that sets
    # cart.customer = null initially.
    # Conservative: inject before the very first <script> that contains "cart".
    # Easier: append a tiny script block at the end of the file.

    # Look for the @endsection at end of content section to inject before it.
    end_marker = "@endsection"
    if s.count(end_marker) >= 1:
        # Inject a small script just before the LAST @endsection
...
'''
        ...
        p.write_text(s)
        print("    UPDATED register/index.blade.php — auto-attaches preAttachCustomer")
    else:
        print("    SKIP register/index.blade.php — no @endsection found, manual wire needed")
PYEOF

cat <<EONOTE

==> Patch 46 applied locally.

Deploy:
  git add -A
  git status
  git commit -m "feat(mobile): walk-in flow MVP (patch 46)"
  git push
...
Smoke test (mobile or narrow desktop):
  1. Open intake tenant on phone-width browser
  2. Look for the + FAB in lower-right (was there before, but now functional)
  3. Tap FAB → "Walk-in" screen opens
  4. Search bar at top, "New customer" + "Quick retail sale" choice tiles, recent customers list
  5. Tap a recent customer → "What's next?" screen with Book/Sale choice
  6. Book → service picker → time picker (uses fallback "next available" slots
     if quick-book/availability endpoint isn't wired yet — that's expected for v1)
  7. Confirm booking → POSTs to /calendar/quick-book → redirects to appointment
  8. Sale → /admin/register with customer pre-attached to cart

Known v1 limitations (deliberately deferred to v1.2+):
  - Time picker uses fallback "next 8 slots" if availability endpoint
    doesn't return data — real availability requires verifying the
    /calendar/quick-book/availability endpoint exists. If not, the
    fallback at least makes the flow demonstrable.
  - Quick-capture form only takes name + phone + (optional) email.
    Full customer profile editing happens after booking.
  - One service per appointment (matches QuickBookController v1).
  - No multi-resource scheduling (uses default resource).

If anything errors, paste me storage/logs/laravel-*.log tail.
EONOTE
