...
@php
  ...
  $radius = ($c['radius'] ?? 'default') === 'default'
              ? ($borderRadius ?? '12px')
              : (['none'=>'0','sm'=>'6px','md'=>'12px','lg'=>'20px'][$c['radius'] ?? ''] ?? '12px');
...
.{{ $uid }}-slide { position:relative; flex:0 0 calc((100% - {{ ($spv - 1) * $gapPx }}px) / {{ $spv }}); scroll-snap-align:start; aspect-ratio: {{ $aspect }}; overflow:hidden; border-radius: {{ $radius }}; background: rgba(255,255,255,.04); border: 0.5px solid var(--mk-border, rgba(255,255,255,.08)); }
...
<section class="{{ $padding ?? 'mk-section--normal' }} {{ $uid }}-sec {{ $customClass }}" @if(!empty($inlineStyle)) style="{{ $inlineStyle }}" @endif @if(!empty($c['anchor_id'])) id="{{ $c['anchor_id'] }}" @endif>
  <div class="mk-container">
    @if($heading !== '' || $sub !== '')
      <div style="margin-bottom:18px">
        @if($heading !== '')<h2 class="mk-section-title" style="margin:0 0 6px">{{ $heading }}</h2>@endif
        @if($sub !== '')<p style="color:var(--mk-muted);margin:0;max-width:60ch">{{ $sub }}</p>@endif
      </div>
    @endif
    ...
    @else
      <div style="aspect-ratio:{{ $aspect }};border-radius:{{ $radius }};background:rgba(255,255,255,.04);display:flex;align-items:center;justify-content:center;font-size:13px;color:var(--mk-muted);opacity:.5">Carousel</div>
    @endif
  </div>
</section>
