...
@php
  ...
  $anchorId  = trim($c['anchor_id'] ?? '') ?: 'rental-browse';
  $custClass = trim($c['custom_classes'] ?? '');
  $instId    = 'p-rbrw-' . ($section->id ?? uniqid());
@endphp
<style>
.{{ $instId }} {
...
<section class="p-section {{ $instId }} {{ $custClass }}" id="{{ $anchorId }}">
  <div class="p-container">
    ...
    <form method="GET" action="#{{ $anchorId }}" style="display:flex;gap:14px;flex-wrap:wrap;align-items:flex-end;justify-content:center;margin-top:28px">
      <div>
            ...
            @foreach($models as $g)
              ...
              <div class="rs-card" style="border-width:1.5px;border-style:solid;border-radius:var(--p-r-lg,14px);padding:20px 22px;display:flex;flex-direction:column">
                ...
                <div style="margin-top:auto;padding-top:16px">
                  <a class="p-btn p-btn--primary" style="width:100%;text-align:center;{{ $rbBtn }}" href="{{ route('tenant.rentals.reserve', ['model' => $m->id, 'starts' => $rbStart->format('Y-m-d\TH:i'), 'due' => $rbDue->format('Y-m-d\TH:i')]) }}">Reserve</a>
                </div>
              </div>
            @endforeach
          </div>
