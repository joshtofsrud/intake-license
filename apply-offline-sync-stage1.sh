#!/bin/bash
# offline-sync-stage-1 — the open register survives an outage.
#   · client_uuid idempotency on tenant_sales (safe offline replay)
#   · offline_sync add-on purchasable on EVERY plan incl. Solo (bundled nowhere)
#   · catalog snapshot endpoint + localStorage cache, offline snapshot search
#   · IndexedDB outbox: cash/manual-tender sales queue offline, auto-sync on
#     reconnect; card & payment-link tenders disabled while offline
# Stage 2 (later): SW shell caching, offline fallback page, cached schedule,
# offline time clock, settings page for snapshot size.
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-OFFLINE-SYNC" routes/web.php; then
  echo "offline-sync-stage-1 already applied — aborting."; exit 1
fi
...
cat > 'database/migrations/2026_07_14_000003_offline_sync_stage1.php' <<'OFS_0_EOF'
<?php
...
cat > 'database/seeders/AddonCatalogSeeder.php' <<'OFS_1_EOF'
<?php
...
cat > 'app/Models/Tenant/TenantSale.php' <<'OFS_2_EOF'
<?php
...
cat > 'app/Services/Tenant/SaleService.php' <<'OFS_3_EOF'
<?php
...
cat > 'app/Http/Controllers/Tenant/RegisterController.php' <<'OFS_4_EOF'
<?php
...
cat > 'resources/views/tenant/register/index.blade.php' <<'OFS_6_EOF'
...
echo "offline-sync-stage-1 applied — server needs: migrate --force, view:clear, route:clear + route:cache"
