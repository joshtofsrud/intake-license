#!/bin/bash
# business-customers-2-tax-register — phase 2 of 3: the money path.
#   · TAX EXEMPTION: SaleService suppresses line tax for a tax-exempt
#     customer and stamps tax_exempt_applied + the certificate onto the sale,
#     so a later edit to the customer cannot rewrite what was true when it was
#     rung up. tax_locked sales are deliberately untouched — they carry
#     pre-computed tax from appointments and must be preserved, and the audit
#     stamp is only written when this pass actually decided the tax.
#   · REGISTER: an exempt customer shows "Tax exempt — cert #…" beneath the
#     tax row, so a $0.00 tax line is never unexplained. A PO-required
#     customer is asked for the number once, before the sale completes,
#     rather than the invoice being rejected weeks later for a missing
#     reference. Cancelling the prompt cancels the completion.
#   · po_number rides all three commit endpoints and is persisted on the sale.
#   · CUSTOMER SEARCH: business_name and contact name/email/phone join the
#     search, and business names rank first — otherwise a business customer
#     is unfindable at the register. The picker payload now carries the
#     display name, exemption, certificate, PO requirement and terms, so the
#     register can behave correctly without another round trip.
#   NOTE: TenantCustomer is not imported in SaleService; the reference is
#   fully qualified deliberately (verified to resolve).
# No routes, no migration (columns shipped in phase 1).
# Server: view:clear.
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-BIZ-TAX" app/Services/Tenant/SaleService.php; then
  echo "phase 2 already applied — aborting."; exit 1
fi
if ! grep -q "MARKER-BIZ-CUSTOMER" app/Models/Tenant/TenantCustomer.php; then
  echo "phase 1 not applied — wrong base, aborting."; exit 1
fi

cat > 'app/Services/Tenant/SaleService.php' <<'BIZ2_0_EOF'
<?php
...
cat > 'app/Models/Tenant/TenantSale.php' <<'BIZ2_1_EOF'
<?php
...
cat > 'app/Http/Controllers/Tenant/CustomerController.php' <<'BIZ2_2_EOF'
<?php
...
cat > 'app/Http/Controllers/Tenant/RegisterController.php' <<'BIZ2_3_EOF'
<?php
...
cat > 'resources/views/tenant/register/index.blade.php' <<'BIZ2_4_EOF'
...
echo "business-customers-2-tax-register applied — server: git pull && php artisan view:clear"
