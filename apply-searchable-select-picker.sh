#!/usr/bin/env bash
set -euo pipefail
# apply-searchable-select-picker.sh — MARKER-SSEL
# Reusable searchable select (approved mockup), wired into the catalog-import
# page for Brand and Category. One component, app CSS vars (follows the tenant
# theme), type-to-filter with match highlight, keyboard nav, "Any …" row,
# count footer. Submits the same brand/category field names via a hidden
# input, so the controller is untouched. Built to reuse later for vendor.
...
if grep -q "MARKER-SSEL" "$VIEW"; then
  echo "Already applied (MARKER-SSEL present) — no-op."
  exit 0
fi

# ---------------------------------------------------------------- component
if [ -f "$COMP" ]; then
  echo "ok   component already present"
else
  cat <<'EOF' > "$COMP"
...
edit("""        <div class="im-field"><label>Brand</label>
...
edit("""        <div class="im-field"><label>Category</label>
...
echo ""
echo "SUCCESS — apply-searchable-select-picker applied."
