#!/bin/bash
# register-recon-and-display — physical register entities + sticky customer pay displays
# New: tenant_registers table, TenantRegister model, RegisterDisplayController,
#      registers management page, public /pay-display/{token} screen.
# Edited: routes, TenantSale (FK relation; legacy varchar register_id replaced),
#      SaleService + RegisterController (sales stamped with register), register UI
#      (picker + display mirroring).
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-REGISTER-RECON-DISPLAY" routes/web.php; then
  echo "register-recon-and-display already applied — aborting."; exit 1
fi
mkdir -p database/migrations app/Models/Tenant app/Http/Controllers/Tenant resources/views/tenant/register

cat > 'database/migrations/2026_07_14_000001_create_tenant_registers_table.php' <<'RRD_0_EOF'
<?php
...
cat > 'app/Models/Tenant/TenantRegister.php' <<'RRD_1_EOF'
<?php
...
cat > 'app/Http/Controllers/Tenant/RegisterDisplayController.php' <<'RRD_2_EOF'
<?php
...
cat > 'app/Models/Tenant/TenantSale.php' <<'RRD_3_EOF'
<?php
...
cat > 'app/Services/Tenant/SaleService.php' <<'RRD_4_EOF'
<?php
...
cat > 'app/Http/Controllers/Tenant/RegisterController.php' <<'RRD_5_EOF'
<?php
...
cat > 'resources/views/tenant/register/registers.blade.php' <<'RRD_7_EOF'
...
cat > 'resources/views/tenant/register/display.blade.php' <<'RRD_8_EOF'
<!DOCTYPE html>
...
cat > 'resources/views/tenant/register/index.blade.php' <<'RRD_9_EOF'
@extends('layouts.tenant.app')
...
echo "register-recon-and-display applied."
