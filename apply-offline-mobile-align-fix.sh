#!/bin/bash
# offline-sync mobile alignment fix — the mobile header centers the brand and
# absolutely-positions the bell (right:10px, 38px wide); the status mount was
# a flex child and rendered UNDERNEATH the bell. Mount is now absolutely
# positioned at right:54px with the same vertical centering as the bell.
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "v=stage8" resources/views/layouts/tenant/app.blade.php; then
  echo "mobile alignment fix already applied — aborting."; exit 1
fi
if ! grep -q "v=stage7" resources/views/layouts/tenant/app.blade.php; then
  echo "gear fix not applied — aborting."; exit 1
fi

cat > 'resources/views/layouts/tenant/_mobile-header.blade.php' <<'MALIGN_0_EOF'
{{-- ================================================================
...
cat > 'resources/views/layouts/tenant/app.blade.php' <<'MALIGN_1_EOF'
<!DOCTYPE html>
...
<script src="{{ asset('js/offline-sync.js') }}?v=stage8"></script>
...
echo "mobile alignment fix applied — view:clear on server, hard-refresh phone"
