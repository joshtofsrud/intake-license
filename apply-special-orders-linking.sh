#!/bin/bash
# special-orders-linking — step 1 of 3. Register-created special orders were
# never cleaned up, and could not be: the order recorded appointment_id and
# customer_id but NOTHING tying it to a sale or line. The register held the
# new order's id on the cart line in the BROWSER only, so removing the line,
# discarding the draft, or abandoning the cart stranded it in "needed".
#   · sale_id + sale_item_id columns on tenant_special_orders
#   · the register sends its draft id when creating an order
#   · removing a line retracts a still-"needed" order immediately (the line
#     knows its id) and warns instead if it is already placed with a vendor —
#     goods may be inbound. Same rule shipped for appointment parts.
#   · discarding a draft or quote retracts all of that sale's needed orders
#   · vendor_id is auto-stamped from the item-vendor catalog's preferred row
#     (falling back to most recently ordered), so orders are groupable by
#     vendor instead of landing with no vendor at all. Scoped through the
#     item because that pivot deliberately carries no tenant_id.
#   · the special-order cancel endpoint now answers JSON when asked, and
#     still redirects for the normal admin form post
# Historical register orders cannot be matched to their sales — that link was
# never recorded — so they surface as "origin unknown" in step 2 rather than
# being guessed at or bulk-cancelled.
...
if grep -q "MARKER-SO-SALE-LINK" app/Services/Tenant/SpecialOrderService.php; then
  echo "special-orders-linking already applied — aborting."; exit 1
fi
if ! grep -q "MARKER-REFUND-QTY" app/Services/Tenant/SaleService.php; then
  echo "wrong base — aborting."; exit 1
fi

cat > 'database/migrations/2026_07_23_000003_add_sale_link_to_tenant_special_orders.php' <<'SOLINK_0_EOF'
<?php
...
cat > 'app/Models/Tenant/TenantSpecialOrder.php' <<'SOLINK_1_EOF'
<?php
...
cat > 'app/Services/Tenant/SpecialOrderService.php' <<'SOLINK_2_EOF'
<?php
...
cat > 'app/Services/Tenant/SaleService.php' <<'SOLINK_3_EOF'
<?php
...
cat > 'app/Http/Controllers/Tenant/RegisterController.php' <<'SOLINK_4_EOF'
<?php
...
cat > 'app/Http/Controllers/Tenant/SpecialOrderController.php' <<'SOLINK_5_EOF'
<?php
...
cat > 'resources/views/tenant/register/index.blade.php' <<'SOLINK_6_EOF'
...
echo "special-orders-linking applied — server: git pull && php artisan migrate --force && php artisan view:clear"
