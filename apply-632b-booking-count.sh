#!/bin/bash
# patch-632b — booking-started overcounting fix.
#   · top-line "Bookings started" now counts DISTINCT sessions (the funnel
#     section already did); per-click beacon events no longer inflate it
#   · choice-page beacon fires once per session via sessionStorage guard
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-PATCH-632B" app/Services/Tenant/TrafficReportService.php; then
  echo "632b already applied — aborting."; exit 1
fi

cat > 'app/Services/Tenant/TrafficReportService.php' <<'P632B_0_EOF'
<?php
// MARKER-PATCH-151A
...
cat > 'resources/views/public/booking-choice.blade.php' <<'P632B_1_EOF'
...
echo "632b applied — view:clear on server"
