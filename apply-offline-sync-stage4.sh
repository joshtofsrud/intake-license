#!/bin/bash
# offline-sync-stage-4 — status moves into the page flow, per approved mockup.
#   · desktop: compact status block in the sidebar nav, above the profile row
#     (amber tint + queue count when offline), gear inline
#   · mobile: the pill is slotted into the mobile header markup (static, not
#     floating) — it can never overlap content
#   · floating fixed pill removed entirely; popover anchors to the tapped gear
#   · cache-buster bumped (?v=stage4) — the stage-3 position fix never showed
#     because the old buster kept serving the cached file
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "v=stage4" resources/views/layouts/tenant/app.blade.php; then
  echo "offline-sync-stage-4 already applied — aborting."; exit 1
fi
if ! grep -q "offline-sync.js" resources/views/layouts/tenant/app.blade.php; then
  echo "stage 3 not applied — aborting."; exit 1
fi

cat > 'public/js/offline-sync.js' <<'OFS4_0_EOF'
...
cat > 'resources/views/layouts/tenant/app.blade.php' <<'OFS4_1_EOF'
<!DOCTYPE html>
...
cat > 'resources/views/layouts/tenant/_sidebar.blade.php' <<'OFS4_2_EOF'
@php
...
cat > 'resources/views/layouts/tenant/_mobile-header.blade.php' <<'OFS4_3_EOF'
{{-- ================================================================
...
echo "offline-sync-stage-4 applied — server needs view:clear, then hard-refresh browsers"
