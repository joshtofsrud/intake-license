#!/bin/bash
# booking-reset-button — quiet "Start over" in the booking step header,
# per approved mockup: dim link right of the progress dots, visible from
# step 2 on, confirm popover before clearing the saved session snapshot
# and reloading fresh. The once-per-session booking_started guard stays.
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "MARKER-BOOKING-RESET" public/js/booking.js; then
  echo "booking reset already applied — aborting."; exit 1
fi

cat > 'resources/views/public/booking.blade.php' <<'BKRESET_0_EOF'
...
cat > 'public/js/booking.js' <<'BKRESET_1_EOF'
/**
...
echo "booking reset applied — view:clear on server; booking.js may need a hard refresh or cache-bust depending on how it is versioned"
