#!/bin/bash
# offline-sync gear fix — desktop gear was near-invisible (13px text glyph)
# and clicks were being swallowed before reaching it.
#   · 18px SVG gear in a 28px hit target, muted color
#   · capture-phase delegated click handler — can't be intercepted
#   · settings popover z-index raised above the sidebar stack
set -e
cd "$(git rev-parse --show-toplevel)"
if grep -q "v=stage7" resources/views/layouts/tenant/app.blade.php; then
  echo "gear fix already applied — aborting."; exit 1
fi
if ! grep -q "MARKER-OFFLINE-SYNC-PIN" app/Http/Middleware/EnsurePinFresh.php; then
  echo "pin-fix not applied — aborting."; exit 1
fi

cat > 'public/js/offline-sync.js' <<'GEARFIX_0_EOF'
...
(function () {
    ...
    mount.innerHTML =
      ...
      'font:600 11.5px Inter,-apple-system,sans-serif;color:' + (quiet ? 'var(--ia-dim,#6e6e6e)' : (b.off ? '#F5C56B' : 'var(--ia-muted,#9c9c9c)')) + '">' +
        ...
        '<button id="ioGearSide" title="Offline sync settings" aria-label="Offline sync settings" ' +
          'style="margin-left:auto;border:none;background:transparent;color:var(--ia-muted,#9c9c9c);cursor:pointer;padding:0;width:28px;height:28px;display:inline-flex;align-items:center;justify-content:center">' +
  ...
  // Capture-phase so nothing else in the page can swallow the gear click.
  document.addEventListener('click', e => {
    const gear = e.target.closest && e.target.closest('#ioGearSide, #ioGearMob');
    if (gear) { e.preventDefault(); e.stopPropagation(); togglePanel(gear); }
  }, true);
  document.addEventListener('click', e => {
...
cat > 'resources/views/layouts/tenant/app.blade.php' <<'GEARFIX_1_EOF'
<!DOCTYPE html>
...
<script src="{{ asset('js/offline-sync.js') }}?v=stage7"></script>
...
echo "gear fix applied — view:clear on server, hard-refresh"
